import { useEffect, useRef } from 'react';
import { gsap } from 'gsap';
import miartzVideo from '../../images/launching 2.mp4';
import styles from './hero.module.css';
import { color, motion } from 'framer-motion';

const Hero = () => {
  const heroRef = useRef<HTMLDivElement>(null);
  const titleRef = useRef<HTMLHeadingElement>(null);
  const subtitleRef = useRef<HTMLParagraphElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        defaults: { ease: 'power3.out' },
        delay: 2.5, // Delay
      });

      // Title
      tl.from(titleRef.current, {
        opacity: 0,
        y: 30,
        duration: 0.8,
      });

      // Subtitle
      tl.from(
        subtitleRef.current,
        {
          opacity: 0,
          y: 20,
          duration: 0.8,
        },
        '-=0.4'
      );
    }, heroRef);

    return () => ctx.revert();
  }, []);

  // Navigation
  const navigate = (destination: string) => {
    window.dispatchEvent(
      new CustomEvent('talentz:navigate', { detail: { to: destination } })
    );
  };

  return (
    <section
      ref={heroRef}
      className="relative h-screen w-screen flex items-center justify-center overflow-hidden"
    >
      {/* Background */}
      <motion.div
        className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 2.5, delay: -4 }}
      >
        <video
          src={miartzVideo}
          autoPlay
          loop
          muted
          playsInline
          className="w-full h-full object-cover" // Blur removed
        />
      </motion.div>

      {/* Glows */}
      <div
        className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full blur-3xl opacity-20 pointer-events-none ${styles.float}`}
        style={{ background: 'var(--gradient-violet)', animationDelay: '1s' }}
      />

      {/* Spline */}
      <motion.div
        className="absolute inset-0 z-0"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1.0, delay: 1.5 }}
      >
        <iframe
          src="https://my.spline.design/nexbotrobotcharacterconcept-QCoQHCf2ON8zVmHKNzgS3JRs/"
          frameBorder="0"
          width="100%"
          height="100%"
          className="w-full h-full"
          title="3D Model"
        />
      </motion.div>

      {/* Content Wrapper */}
      <div
        className={`relative z-10 container mx-auto px-4 pointer-events-none ${styles.contentWrapper}`}
      >
        {/* Column 1: Title */}
        <h1 ref={titleRef} className={styles.title}>
          Connecting <span className={styles.highlightText}>Talent</span> with
          Opportunity
        </h1>

        {/* Column 2: Right Content */}
        <div className={styles.rightColumn}>
          {/* Subtitle */}
          <p ref={subtitleRef} className={styles.subtitle}>
            Discover. Create. Collaborate.<br />
            MIARTZ brings your imagination to{" "}
            <span className={styles.highlightText1}>Life</span>
          </p>


          {/* Button Group */}
          <div className={styles.buttonGroup}>
            {/* Button 1 */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 4.1 }}
              className={styles.buttonWrapper}
              style={{ pointerEvents: 'auto' }}
            >
              <button
                className={`${styles.btn} ${styles.btnBubble} ${styles.btnPrimary}`}
                onClick={() => navigate('App1')}
              >
                Get Started
              </button>
            </motion.div>

            {/* === BUTTON 2 === */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 4 }}
              style={{ pointerEvents: 'auto' }}
            >
              <div className={styles.buttonContainer}>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <div className={styles.area}></div>
                <button
                  className={styles.button}
                  onClick={() => navigate('features')}
                >
                  <div className={styles.base}></div>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 52 42"
                    height="25"
                    width="50"
                    className={styles.filament}
                  >
                    <path
                      strokeWidth="3"
                      stroke="var(--filament)"
                      d="M1 11.5153C10.1667 12.6819 30.7 14.8153 39.5 14.0153C48.3 13.2153 50.1667 10.3486 50 9.01525C49.8333 6.84859 48.6 2.41525 45 2.01525C41.4 1.61525 39.8333 9.18192 39.5 13.0153V29.5153C39.5 32.5153 42 40.0153 45 40.0153C48 40.0153 50 37.5153 50 35.5153C50 33.5153 47.7 29.0153 38.5 27.0153C29.3 25.0153 9.66667 27.8486 1 29.5153"
                    ></path>
                  </svg>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 52 42"
                    height="25"
                    width="50"
                    className={`${styles.filament} ${styles.filamentGlow}`}
                  >
                    <defs>
                      <linearGradient
                        id="strokeGradient"
                        x1="0%"
                        y1="0%"
                        x2="100%"
                        y2="0%"
                      >
                        <stop offset="0%" stopColor="var(--filament)"></stop>
                        <stop offset="100%" stopColor="var(--filament)"></stop>
                      </linearGradient>
                    </defs>
                    <path d="M1 11.5153C10.1667 12.6819 30.7 14.8153 39.5 14.0153C48.3 13.2153 50.1667 10.3486 50 9.01525C49.8333 6.84859 48.6 2.41525 45 2.01525C41.4 1.61525 39.8333 9.18192 39.5 13.0153V29.5153C39.5 32.5153 42 40.0153 45 40.0153C48 40.0153 50 37.5153 50 35.5153C50 33.5153 47.7 29.0153 38.5 27.0153C29.3 25.0153 9.66667 27.8486 1 29.5153"></path>
                  </svg>
                  <div className={styles.glow}></div>
                  <div className={styles.lightningMarquee}>
                    <div>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                    </div>
                    <div>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                      <svg
                        viewBox="0 0 16 16"
                        className={styles.lightning}
                        fill="var(--glow)"
                        height="15"
                        width="15"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M11.251.068a.5.5 0 0 1 .227.58L9.677 6.5H13a.5.5 0 0 1 .364.843l-8 8.5a.5.5 0 0 1-.842-.49L6.323 9.5H3a.5.5 0 0 1-.364-.843l8-8.5a.5.5 0 0 1 .615-.09z"></path>
                      </svg>
                    </div>
                  </div>
                  <p className={styles.text}>
                    <span style={{ '--i': 0 }}>Explore</span>
                  </p>
                </button>
                <div className={styles.bulbWrapper}>
                  <div className={styles.bulb}></div>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.select}`}
                    stroke="#000"
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M14.082 2.182a.5.5 0 0 1 .103.557L8.528 15.467a.5.5 0 0 1-.917-.007L5.57 10.694.803 8.652a.5.5 0 0 1-.006-.916l12.728-5.657a.5.5 0 0 1 .556.103z"></path>
                  </svg>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.move}`}
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M7.646.146a.5.5 0 0 1 .708 0l2 2a.5.5 0 0 1-.708.708L8.5 1.707V5.5a.5.5 0 0 1-1 0V1.707L6.354 2.854a.5.5 0 1 1-.708-.708zM8 10a.5.5 0 0 1 .5.5v3.793l1.146-1.147a.5.5 0 0 1 .708.708l-2 2a.5.5 0 0 1-.708 0l-2-2a.5.5 0 0 1 .708-.708L7.5 14.293V10.5A.5.5 0 0 1 8 10M.146 8.354a.5.5 0 0 1 0-.708l2-2a.5.5 0 1 1 .708.708L1.707 7.5H5.5a.5.5 0 0 1 0 1H1.707l1.147 1.146a.5.5 0 0 1-.708.708zM10 8a.5.5 0 0 1 .5-.5h3.793l-1.147-1.146a.5.5 0 0 1 .708-.708l2 2a.5.5 0 0 1 0 .708l-2 2a.5.5 0 0 1-.708-.708L14.293 8.5H10.5A.5.5 0 0 1 10 8"></path>
                  </svg>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.rotate}`}
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2z"></path>
                    <path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466"></path>
MAIN:            </svg>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.hammer}`}
                    stroke="#000"
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M9.972 2.508a.5.5 0 0 0-.16-.556l-.178-.129a5 5 0 0 0-2.076-.783C6.215.862 4.504 1.229 2.84 3.133H1.786a.5.5 0 0 0-.354.147L.146 4.567a.5.5 0 0 0 0 .706l2.571 2.579a.5.5 0 0 0 .708 0l1.286-1.29a.5.5 0 0 0 .146-.353V5.57l8.387 8.873A.5.5 0 0 0 14 14.5l1.5-1.5a.5.5 0 0 0 .017-.689l-9.129-8.63c.747-.456 1.772-.839 3.112-.839a.5.5 0 0 0 .472-.334"></path>
                  </svg>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.lightbulbOff}`}
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M2 6a6 6 0 1 1 10.174 4.31c-.203.196-.359.4-.453.619l-.762 1.769A.5.5 0 0 1 10.5 13a.5.5 0 0 1 0 1 .5.5 0 0 1 0 1l-.224.447a1 1 0 0 1-.894.553H6.618a1 1 0 0 1-.894-.553L5.5 15a.5.5 0 0 1 0-1 .5.5 0 0 1 0-1 .5.5 0 0 1-.46-.302l-.761-1.77a2 2 0 0 0-.453-.618A5.98 5.98 0 0 1 2 6m6-5a5 5 0 0 0-3.479 8.592c.263.254.514.564.676.941L5.83 12h4.342l.632-1.467c.162-.377.413-.687.676-.941A5 5 0 0 0 8 1"></path>
                  </svg>
                  <svg
                    viewBox="0 0 16 16"
                    className={`${styles.cursor} ${styles.lightbulbOn}`}
                    fill="#fff"
                    height="30"
                    width="30"
                    xmlns="http://www.w3.org/2all000/svg"
                  >
                    <path d="M2 6a6 6 0 1 1 10.174 4.31c-.203.196-.359.4-.453.619l-.762 1.769A.5.5 0 0 1 10.5 13h-5a.5.5 0 0 1-.46-.302l-.761-1.77a2 2 0 0 0-.453-.618A5.98 5.98 0 0 1 2 6m3 8.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1l-.224.447a1 1 0 0 1-.894.553H6.618a1 1 0 0 1-.894-.553L5.5 15a.5.5 0 0 1-.5-.5"></path>
                  </svg>
                </div>
              </div>
            </motion.div>
            {/* === END NEW BUTTON 1 === */}
          </div>
        </div>
        {/* End rightColumn */}
      </div>
      {/* End contentWrapper */}
    </section>
  );
};

export default Hero;